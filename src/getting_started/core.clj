...
(defn game [term]
  ...
  (while (not= @quitloop true)
    (do
      ...
      (if (and (not= @quitloop "no") (not= @presskey :enter)) (text_display term [[1 5] @empty_string [1 5] (:name @player-map)]) ())))
      ...
      (while (= @reloop false)
        (do
          ...
          (text_display term [[1 1] "Press Z or UP to MOVE" [1 2] "Press Q or LEFT to turn LEFT" [1 3] "Press D or RIGHT to turn RIGHT"
                              ...
                              [1 7] (str "You look to the " (:nswe @player-map)) [1 8]
                              (str "in X:" (:position_x @player-map) " Y:" (:position_y @player-map)) [1 10]])
          ...
          (while (= @moveloop false)
            (do
              ...
              (cond
                (or (= @presskey \z) (= @presskey :up)) [(reset! to_display (move_forward)) (if (= @to_display "watch out edge")
